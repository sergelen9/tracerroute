...
def main():
    ...
    no_reply = '*'
    count = 0
    insert_db_dict = {'address': '', 'delay_number': '', 'hop_number': ''}
    for ttl, seq_results in enumerate(results):
        insert_db_dict['hop_number'] = ttl + 1
        print('%2d ' % (ttl + 1), end='')
        prev_addr = None
        for result in seq_results:
            ...
            if addr != no_reply:
                if count == 0:
                    ...
                    avg_delay = delay * 1e3
                    insert_db_dict['delay_number'] = str(avg_delay)[:6]
                    print('\t  %0.3f ms' % avg_delay, end='')
            count = count + 1
            prev_addr = addr
